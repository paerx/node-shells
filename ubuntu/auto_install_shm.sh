...
#read -p "What base directory should the node use (default ~/.shardeum): "
...
while :; do
#  read -p "Enter the port (1025-65536) to access the web based Dashboard (default $DASHPORT_DEFAULT): "
  DASHPORT=8081
...
while :; do
  #read -p "If you wish to set an explicit external IP, enter an IPv4 address (default=$EXTERNALIP_DEFAULT): "
  EXTERNALIP="auto"
...
while :; do
  #read -p "If you wish to set an explicit internal IP, enter an IPv4 address (default=$INTERNALIP_DEFAULT): "
  INTERNALIP="auto"
...
#  read -p "This allows p2p communication between nodes. Enter the first port (1025-65536) for p2p communication (default $SHMEXT_DEFAULT): "
  SHMEXT="9001"
  ...
  #read -p "Enter the second port (1025-65536) for p2p communication (default $SHMINT_DEFAULT): "
  SHMINT="10001"
